-- ============================================================================
-- AWS SPOT OPTIMIZER - COMPLETE DATABASE SCHEMA v1.4.0 (PRODUCTION READY)
-- ============================================================================
...
-- Enhanced: Performance indexes, statistics, exports, helper functions
-- ============================================================================
...
DROP PROCEDURE IF EXISTS get_price_history;
DROP PROCEDURE IF EXISTS get_instance_metrics;

-- Drop functions
DROP FUNCTION IF EXISTS calculate_savings_percent;
DROP FUNCTION IF EXISTS get_agent_uptime_hours;
DROP FUNCTION IF EXISTS calculate_hourly_savings;
DROP FUNCTION IF EXISTS estimate_monthly_savings;

-- Drop views
DROP VIEW IF EXISTS v_active_instances;
DROP VIEW IF EXISTS v_client_summary;
DROP VIEW IF EXISTS v_recent_switches;
DROP VIEW IF EXISTS v_instance_health;
DROP VIEW IF EXISTS v_agent_performance;
DROP VIEW IF EXISTS v_pool_price_stats;
...
CREATE TABLE IF NOT EXISTS instances (
  ...
  INDEX idx_instances_active_mode (is_active, current_mode, client_id),
  INDEX idx_instances_search (id, instance_type, client_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
...
CREATE TABLE IF NOT EXISTS spot_price_snapshots (
  ...
  INDEX idx_spot_price_pool_time (pool_id, captured_at DESC)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
...
-- FIXED: Renamed 'trigger' column to 'event_trigger' (reserved keyword)
CREATE TABLE IF NOT EXISTS switch_events (
  ...
  INDEX idx_switch_events_trigger_time (event_trigger, timestamp DESC),
  INDEX idx_switch_events_agent_savings (agent_id, timestamp DESC, savings_impact)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
...
-- View: Instance health summary (useful for dashboards)
CREATE OR REPLACE VIEW v_instance_health AS
SELECT 
  i.id,
  i.client_id,
  i.agent_id,
  ...
  i.current_mode,
  i.is_active,
  TIMESTAMPDIFF(HOUR, i.last_switch_at, NOW()) as hours_in_current_pool,
  (SELECT COUNT(*) FROM switch_events se WHERE (se.old_instance_id = i.id OR se.new_instance_id = i.id) 
   AND se.timestamp >= DATE_SUB(NOW(), INTERVAL 7 DAY)) as switches_last_week,
  (SELECT risk_score FROM risk_scores WHERE instance_id = i.id ORDER BY created_at DESC LIMIT 1) as latest_risk_score,
  CASE 
    WHEN i.current_mode = 'spot' THEN 'healthy'
    WHEN i.current_mode = 'ondemand' THEN 'needs_optimization'
    ELSE 'unknown'
...
SELECT 
  ...
  TIMESTAMPDIFF(HOUR, a.created_at, NOW()) as uptime_hours
FROM agents a
...
GROUP BY a.id, a.client_id, a.status, a.enabled, a.auto_switch_enabled, a.instance_count,
         ac.min_savings_percent, ac.risk_threshold, ac.max_switches_per_week, a.created_at;

-- View: Pool price statistics (useful for pricing comparisons)
CREATE OR REPLACE VIEW v_pool_price_stats AS
SELECT 
  sp.id as pool_id,
  sp.instance_type,
  sp.region,
  ...
  STDDEV(sps.price) as price_stddev,
  MAX(sps.captured_at) as latest_capture
FROM spot_pools sp
...
-- Calculate hourly savings rate
CREATE FUNCTION calculate_hourly_savings(
  p_ondemand_price DECIMAL(12,6),
  p_spot_price DECIMAL(12,6)
)
RETURNS DECIMAL(12,6)
DETERMINISTIC
BEGIN
  IF p_ondemand_price > 0 AND p_spot_price >= 0 THEN
    RETURN p_ondemand_price - p_spot_price;
  ELSE
    RETURN 0.000000;
  END IF;
END//

-- Get monthly savings estimate
CREATE FUNCTION estimate_monthly_savings(
  p_hourly_savings DECIMAL(12,6)
)
RETURNS DECIMAL(18,4)
DETERMINISTIC
BEGIN
  -- 24 hours * 30 days = 720 hours per month
  RETURN p_hourly_savings * 720;
END//
...
-- Calculate monthly savings for a client
...
CREATE PROCEDURE get_price_history(
  IN p_pool_id VARCHAR(128),
  IN p_days INT,
...
BEGIN
  IF p_interval = 'hour' THEN
    ...
    WHERE pool_id = p_pool_id
      AND captured_at >= DATE_SUB(NOW(), INTERVAL p_days DAY)
    GROUP BY DATE_FORMAT(captured_at, '%Y-%m-%d %H:00:00')
    ORDER BY time_bucket ASC;
  ELSE
    -- Daily aggregation
    SELECT 
      DATE_FORMAT(captured_at, '%Y-%m-%d') as time_bucket,
      AVG(price) as avg_price,
      MIN(price) as min_price,
      MAX(price) as max_price,
      COUNT(*) as sample_count
    ...
    WHERE pool_id = p_pool_id
      AND captured_at >= DATE_SUB(NOW(), INTERVAL p_days DAY)
    GROUP BY DATE_FORMAT(captured_at, '%Y-%m-%d')
    ORDER BY time_bucket ASC;
  END IF;
END//

-- Get instance metrics summary
CREATE PROCEDURE get_instance_metrics(IN p_instance_id VARCHAR(64))
BEGIN
  SELECT 
    i.id,
    i.instance_type,
    ...
    COUNT(DISTINCT se.id) as total_switches,
    COUNT(DISTINCT CASE WHEN se.timestamp >= DATE_SUB(NOW(), INTERVAL 7 DAY) THEN se.id END) as switches_last_7_days,
    COUNT(DISTINCT CASE WHEN se.timestamp >= DATE_SUB(NOW(), INTERVAL 30 DAY) THEN se.id END) as switches_last_30_days,
    COALESCE(SUM(CASE WHEN se.timestamp >= DATE_SUB(NOW(), INTERVAL 30 DAY) THEN se.savings_impact ELSE 0 END), 0) as savings_last_30_days,
    COALESCE(SUM(se.savings_impact), 0) as total_savings
  FROM instances i
  LEFT JOIN switch_events se ON (se.old_instance_id = i.id OR se.new_instance_id = i.id)
  WHERE i.id = p_instance_id
  GROUP BY i.id, i.instance_type, i.current_mode, i.current_pool_id, 
           i.spot_price, i.ondemand_price, i.baseline_ondemand_price,
           i.installed_at, i.last_switch_at;
END//
...
-- Update client total savings on switch event
...
-- Validate baseline price on instance insert
...
-- Check all indexes including new performance indexes
SELECT 
  TABLE_NAME,
  INDEX_NAME,
...
SELECT 
  'Schema v1.4.0 deployed successfully!' as status,
  ...
  (SELECT COUNT(*) FROM information_schema.ROUTINES 
   WHERE ROUTINE_SCHEMA = 'spot_optimizer' 
   AND ROUTINE_TYPE = 'FUNCTION') as total_functions,
  (SELECT COUNT(*) FROM information_schema.VIEWS 
   WHERE TABLE_SCHEMA = 'spot_optimizer') as total_views,
  (SELECT COUNT(*) FROM information_schema.TRIGGERS 
   WHERE TRIGGER_SCHEMA = 'spot_optimizer') as total_triggers,
  (SELECT COUNT(*) FROM information_schema.EVENTS 
   WHERE EVENT_SCHEMA = 'spot_optimizer') as total_events;

-- ============================================================================
-- END OF COMPLETE SCHEMA v1.4.0
-- ============================================================================